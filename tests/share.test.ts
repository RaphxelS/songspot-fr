...
describe("lib/share — clipboard", () => {
  beforeEach(() => {
    Object.defineProperty(window, "navigator", { value: { clipboard: { writeText: vi.fn().mockResolvedValue(undefined) }, userAgent: "test" }, writable: true, configurable: true });
  });
  afterEach(() => { vi.restoreAllMocks(); });
  it("copyShareUrl appelle clipboard", async () => {
    const mockWrite = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, "clipboard", { value: { writeText: mockWrite }, writable: true, configurable: true });
    const ok = await copyShareUrl("abc123", "Facile", "https://example.com");
    expect(mockWrite).toHaveBeenCalledTimes(1);
    const calledUrl = mockWrite.mock.calls[0][0];
